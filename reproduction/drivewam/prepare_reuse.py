#!/usr/bin/env python3
"""Partition inputs into reusable DriveWAM rows and new rows."""
...
def main() -> None:
    ...
    parser.add_argument("--benchmark-inputs", type=Path, required=True)
    ...
    for benchmark_index, row in enumerate(benchmark_rows):
        key = row["source_key"]
        ...
        source = args.benchmark_inputs / f"sample_{benchmark_index:06d}.pkl"
    ...
    summary = {"benchmark": len(benchmark_rows), "reused": len(reuse), "new": len(new_rows)}
