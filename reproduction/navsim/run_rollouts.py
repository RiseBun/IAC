...
def run(rows: list[dict[str, Any]], cache_root: Path, *, horizon_s: float, interval_s: float, success_threshold: float) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            "Independent execution validation requires the official NAVSIM/PDM runtime checkout (not the "
            ...
            enriched.update(
                {
                    ...
                    "task_success_source": "navsim_pdm_score",
    ...
    summary = {
        "protocol": "navsim-counterfactual-realized-rollout",
        "input_rows": len(rows),
