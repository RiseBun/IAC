    ...
    *,
    ...
    return {
        "protocol": (
            ...
            if "optimizer-internal-longitudinal-residual" in longitudinal_models
            else "continuous-foresight-action-level1-longitudinal"
            if longitudinal_models
            else "continuous-foresight-action-level1"
        ),
        "evidence_scope": (
...
def main() -> None:
    ...
    if args.longitudinal_calibration is not None:
        ...
        if calibration.get("protocol") != "longitudinal-residual-calibration":
    ...
    if args.pose_calibration is not None:
        ...
        if pose_calibration.get("protocol") != "continuous-se2-pose-calibration":
