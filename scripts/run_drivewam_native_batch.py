...
def _install_flash_attention_import_fallback() -> bool:
    """Allow DriveWAM's torch-SDPA evaluator without flash-attn installed."""
    try:
...
def _action_trajectory(batch_item, *, mode: str, expected_steps: int) -> torch.Tensor:
    ...
    if mode in {"left", "right"}:
        ...
        values[:, :2] = 0.0
        values[:, 2] = 0.0
    elif mode in {"slow", "normal", "fast"}:
        scale = {"slow": 0.5, "normal": 1.0, "fast": 1.5}[mode]
        values[:, 0] = values[0, 0] + scale * (values[:, 0] - values[0, 0])
...
def main():
    ...
    ap.add_argument(
        "--intervention",
        choices=("predict", "logged", "left", "right", "stop", "joint_left", "joint_right", "joint_stop", "joint_slow", "joint_normal", "joint_fast"),
        default="predict",
    ...
    ap.add_argument(
        "--command-override", choices=("left", "straight", "right", "unknown"),
        default=None,
        help="override only the high-level navigation command; native action remains model-generated",
    )
    args = ap.parse_args()

    drivewam = "/mnt/slurmfs-4090node1/homes/zchen897/DriveWAM"
    tools = "/mnt/slurmfs-4090node1/homes/zchen897/iac_new/tools"
    iac_src = str(Path(__file__).resolve().parents[1] / "src")
    _install_flash_attention_import_fallback()
    ...
    if args.intervention != "predict":
        from iac_new.drivewam_adapter import (
            DriveWAMIntervention,
            rollout_external_action,
            rollout_joint_external_action,
        )
    ...
    for idx in range(min(len(dataset), args.num_samples)):
        ...
        if args.command_override is not None:
            command_id = {"left": 0, "straight": 1, "right": 2, "unknown": 3}[args.command_override]
            batch_item["command_id"] = torch.tensor(command_id, dtype=torch.long)
            batch_item["prompt"] = config.command_prompts[command_id]
        ...
        with torch.inference_mode():
            if args.intervention == "predict":
                ...
                action = _action_trajectory(
                    raw_sample,
                    mode=(args.intervention.removeprefix("joint_") if args.intervention.startswith("joint_") else args.intervention),
                    expected_steps=int(config.action_chunk_steps),
                )
                if args.intervention.startswith("joint_"):
                    pred_actions, pred_latents = rollout_joint_external_action(
                        evaluator,
                        batch,
                        action,
        ...
        rows.append({
            ...
            "intervention_variant": (
                "repository_predict" if args.intervention == "predict"
                    ...
                    "joint_external_action_condition"
                    if args.intervention.startswith("joint_")
