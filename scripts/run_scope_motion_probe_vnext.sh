...
CONFIG="${CONFIG:-configs/train_navsim_future_dinov2_scope_motion_probe_vnext.py}"
BASE_CKPT="${BASE_CKPT:-work_dirs/iac_navsim_future_dinov2_supported_set_listwise_vnext/checkpoints/latest.pth}"
WORK_DIR="${WORK_DIR:-work_dirs/iac_navsim_future_dinov2_scope_motion_probe_vnext}"
EPOCHS="${EPOCHS:-7}"
MAX_TRAIN_STEPS="${MAX_TRAIN_STEPS:-300}"
MAX_VAL_STEPS="${MAX_VAL_STEPS:-60}"
...
for control in normal reverse_future shuffle_future zero_future; do
...
root = Path(os.environ.get("WORK_DIR", "work_dirs/iac_navsim_future_dinov2_scope_motion_probe_vnext"))
...
for path in sorted(eval_dir.glob("*_summary.json")):
    ...
    rows.append({
        ...
        "hard_mismatch_energy_above_gt_group_rate": data.get("hard_mismatch_energy_above_gt_group_rate"),
        "near_perturb_energy_above_gt_group_rate": data.get("near_perturb_energy_above_gt_group_rate"),
...
(eval_dir / "summary_table.json").write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
for row in rows:
    print(row)
PY
