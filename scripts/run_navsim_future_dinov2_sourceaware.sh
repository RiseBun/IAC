...
CONFIG="${CONFIG:-configs/train_navsim_future_dinov2_sourceaware.py}"
WORK_DIR="${WORK_DIR:-work_dirs/iac_navsim_future_dinov2_sourceaware}"
...
MAX_VAL_STEPS="${MAX_VAL_STEPS:-120}"
...
"$PYTHON_BIN" tools/audit_consistency_index.py \
  indices_navsim_future/consistency_train.jsonl \
...
"$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" \
  "${TRAIN_ARGS[@]}" 2>&1 | tee "$WORK_DIR/train.log"

if [[ -z "$EVAL_CHECKPOINT" ]]; then
  EVAL_CHECKPOINT="$WORK_DIR/checkpoints/latest.pth"
fi
