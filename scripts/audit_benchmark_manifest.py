...
def audit_public(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Audit the redacted public identity manifest without requiring private data."""
    ...
    for index, row in enumerate(rows):
        ...
        if not sample_id or not benchmark_id or not row.get("source_key"):
            issues.append({"row": index, "field": "identity", "reason": "sample_id, benchmark_id and source_key required"})
        times = row.get("future_times_s")
        if not isinstance(times, list) or len(times) < 4:
            issues.append({"row": index, "field": "future_times_s", "reason": "at least four public future timestamps required"})
        elif any(float(times[i]) >= float(times[i + 1]) for i in range(len(times) - 1)):
            issues.append({"row": index, "field": "future_times_s", "reason": "future timestamps must increase"})
        history_ids = row.get("history_frame_ids")
        if not isinstance(history_ids, list) or len(history_ids) != 4:
            issues.append({"row": index, "field": "history_frame_ids", "reason": "four redacted history frame ids required"})
        leaked = sorted(forbidden & row.keys())
        if leaked:
            issues.append({"row": index, "field": "private_fields", "reason": "private fields present", "values": leaked})
    for field, values in (("sample_id", sample_ids), ("benchmark_id", benchmark_ids)):
        duplicates = sorted(value for value, count in Counter(values).items() if value and count > 1)
        if duplicates:
            issues.append({"field": field, "reason": "duplicate identity", "values": duplicates[:20]})
    return {
        "protocol": "iac-public-benchmark-audit",
        "rows": len(rows),
        "scene_groups": len({str(row.get("scene_group")) for row in rows}),
        "strata": dict(sorted(Counter(str(row.get("stratum")) for row in rows).items())),
        "issues": issues[:200],
        "issue_count": len(issues),
        "public_manifest_ready": bool(rows) and not issues,
    }
...
def audit(rows: list[dict[str, Any]], *, require_realized: bool, require_success: bool) -> dict[str, Any]:
    ...
    return {
        "protocol": "iac-wam-benchmark-manifest-audit",
        "rows": len(rows),
        "groups": len(groups),
...
def main() -> None:
    ...
    parser.add_argument("--public", action="store_true", help="audit a redacted public manifest")
    ...
    result = (
        audit_public(rows)
        if args.public
        else audit(rows, require_realized=args.require_realized_state, require_success=args.require_task_success)
    )
    if args.other_split:
