...
def main() -> None:
    ...
    for source, branches in sorted(by_source.items()):
        ...
        for control, (expected_left, expected_right) in controls.items():
            ...
            rows.append({"source_key": source, "control": control, **{key: score[key] for key in ("status_counts", "interval_coverage", "median_residual_px", "median_observed_delta_px", "median_expected_delta_px", "median_direction_cosine", "median_direction_vector_fraction", "temporal_persistence", "median_support_fraction", "minimum_support_fraction")}})
    summary = {"protocol": "iac-twin-differential-forward-consistency-v1", "min_vector_norm_px": args.min_vector_norm, "controls": {control: {key: (float(np.median([row[key] for row in rows if row["control"] == control and row[key] is not None])) if any(row["control"] == control and row[key] is not None for row in rows) else None) for key in ("interval_coverage", "median_residual_px", "median_observed_delta_px", "median_expected_delta_px", "median_direction_cosine", "median_direction_vector_fraction", "temporal_persistence", "median_support_fraction", "minimum_support_fraction")} for control in ("normal", "reversed", "zero")}}
