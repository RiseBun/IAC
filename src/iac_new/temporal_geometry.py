...
def fit_causal_road_plane(
    *, observed_flows: np.ndarray, history_ego_state: np.ndarray,
    ...
    return {"protocol": "causal-road-plane-calibration",
...
def estimate_history_flow_scale(
    *,
    ...
    if not 0.0 <= correction_shrinkage <= 1.0:
        raise ValueError("correction_shrinkage must be in [0, 1]")
    poses = [se2_to_transform(*row[:3]) @ camera for row in states]
    ratios: list[float] = []
    ...
    if not ratios:
        return {
            "protocol": "history-only-persistent-flow-scale",
            "available": False,
    ...
    correction = float(np.clip(
        np.exp(-float(correction_shrinkage) * np.log(max(scale, 1e-6))),
        0.8,
        1.25,
    ))
    return {
...
class TemporalScaleCalibrator:
    ...
    def estimate(
        ...
        *,
        ...
        return {
            "protocol": "temporal-scale-calibration",
...
class HomographyBoundaryPropagator:
    ...
    def update(
        ...
        *,
        ...
        return {
            "protocol": f"{self.propagation_method}-boundary-propagation",
