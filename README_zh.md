...
置信区间；不可测样本统一记为 `unavailable`，不填零。
...
MAS-yaw 与 RCS-yaw 均在两个模型上通过冻结门槛（成对覆盖率 ≥90%，source
bootstrap 方向准确率下界 ≥75%）。GS 使用 logged future 独立校准，并通过了
source-disjoint 校准和身份置换控制。这些正式分数是结构/方向性的：横向位移、
