#!/usr/bin/env python3
"""Select a scene-disjoint, all-NAVSIM benchmark manifest.
...
def main() -> None:
    ...
    for row in rows:
        row["_stratum"] = classify(row)
        row["_scene_group"] = scene_group(row)

    by_scene: dict[str, list[dict]] = defaultdict(list)
    for row in rows:
        if row["_stratum"] != "unknown":
            by_scene[row["_scene_group"]].append(row)
    ...
    available = Counter(r["_stratum"] for r in candidates)
    ...
    def nonoverlap(row: dict) -> bool:
        ...
        return all(abs(start - prev) > 11 for prev in used_intervals[row["_scene_group"]])

    def add_if_valid(row: dict) -> bool:
        if not nonoverlap(row):
            return False
        selected.append(row)
        used_intervals[row["_scene_group"]].append(int(row.get("frame_idx", 0)))
        return True

    for stratum in ("acceleration", "braking", "lateral_turn", "straight_cruise", "stop"):
        pool = [r for r in candidates if r["_stratum"] == stratum]
    ...
    for stratum in preference:
        ...
        for row in pool:
            if len(selected) >= args.total:
                break
            if stratum == "stop" and sum(r["_stratum"] == "stop" for r in selected) >= stop_cap:
                break
            add_if_valid(row)
        if len(selected) >= args.total:
            break

    selected.sort(key=lambda r: (r["_stratum"], r["source_key"]))
    clean = []
    ...
    if len(clean) != args.total:
        raise RuntimeError(
            f"benchmark selection produced {len(clean)} rows, expected exactly {args.total}"
        )
    sample_ids = [str(row["sample_id"]) for row in clean]
    if len(set(sample_ids)) != len(sample_ids):
        raise RuntimeError("benchmark selection produced duplicate sample_id values")

    args.output_root.mkdir(parents=True, exist_ok=True)
    out_path = args.output_root / "benchmark_navsim_private.jsonl"
    out_path.write_text("".join(json.dumps(r, ensure_ascii=False, separators=(",", ":")) + "\n" for r in clean))
    audit = {
        "protocol": "iac-benchmark-navsim-only",
    ...
    (args.output_root / "benchmark_navsim_audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
