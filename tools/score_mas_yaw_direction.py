...
def score(manifest, flows, orientation, action_threshold, visual_threshold, action_sign=1, visual_sign=1, draws=20000):
    ...
    branch_available = sum(r["available"] for r in rows)
    return {"branch_count":branch_count,"branch_available":branch_available,"branch_coverage":branch_available/branch_count if branch_count else 0.0,"pair_count":len(grouped),"pair_available":len(pair_rows),"pair_coverage":len(pair_rows)/len(grouped) if grouped else 0.0,"direction_hits":hits,"direction_n":n,"direction_accuracy":hits/n if n else None,"direction_ci95":wilson(hits,n),"source_bootstrap_ci95":list(np.quantile(means,[.025,.975])) if means else None,"conditional_score":hits/n if n else None,"score_coverage":n/branch_count if branch_count else None,"status_counts":{"scored":n,"abstain":max(branch_count-n,0),"unavailable":0},"abstention_reasons":{"below_action_or_visual_deadband":max(branch_count-n,0)},"pairs":pair_rows,"branches":rows}
